...
"""Generate the v3.20 Anastig contract + final model matrix from capabilities.

Pure projection of ``model_capabilities()``. Emits a complete primary partition of
the 151 models plus overlapping task-specific live views, with UI-copy guidance.

Outputs:
  docs/anastig_model_allowlist_v320.json
  docs/anastig_model_contract_v320.md
  docs/qa/v320_final_operationalization/final_model_matrix.json
  docs/qa/v320_final_operationalization/final_model_matrix.md
"""
...
DOCS = REPO / "docs"
QA = REPO / "docs" / "qa" / "v320_final_operationalization"

# Every bucket Anastig consumes. The first group is a disjoint PRIMARY PARTITION
# (each model lands in exactly one); the second group is overlapping live VIEWS.
PRIMARY_BUCKETS = [
    "train_ready_live",
...
def main() -> int:
    ...
    (DOCS / "anastig_model_allowlist_v320.json").write_text(
        json.dumps(
            {
    ...
    for b in PRIMARY_BUCKETS + VIEW_BUCKETS:
        ...
        md.append("")
    (DOCS / "anastig_model_contract_v320.md").write_text("\n".join(md) + "\n")
        ...
        rows.append(
            {
                ...
                "reload_live": c["reload_live_verified"],
                "export_live": c["export_live_verified"],
    ...
    print(
        f"[v320] contract + matrix for {len(rows)} models | primary partition total={primary_total}"
    )
    print(
        "[v320] live: inference={} train={} finetune={}".format(
