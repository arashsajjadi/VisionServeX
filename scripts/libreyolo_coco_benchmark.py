...
for _size in ("n", "s", "m", "l", "x"):
    MODEL_MAP[f"libreyolo-dfine-{_size}-seg"] = ("LibreDFINE", f"{_size}-seg", f"LibreDFINE{_size}-seg")
for _size in ("n", "s", "m", "l"):
    MODEL_MAP[f"libreyolo-rfdetr-{_size}-seg"] = ("LibreYOLORFDETR", f"{_size}-seg", f"LibreRFDETR{_size}-seg")
for _size in ("r18", "r34", "r50", "r50m", "r101"):
    MODEL_MAP[f"libreyolo-rtdetr-{_size}-seg"] = ("LibreYOLORTDETR", f"{_size}-seg", f"LibreRTDETR{_size}-seg")
for _size in ("n", "t", "s", "m", "l", "x"):
    MODEL_MAP[f"libreyolo-yolox-{_size}-seg"] = ("LibreYOLOX", f"{_size}-seg", f"LibreYOLOX{_size}-seg")
...
def _run_model(model_id: str, coco_gt, img_ids, img_paths, cat_map) -> dict:
    ...
    try:
        ...
        print(f"  [{model_id}] mAP50:95={ev.stats[0]:.4f} AP50={ev.stats[1]:.4f} lat_p50={p50:.0f}ms fps={fps:.0f}")
    except Exception as e:
