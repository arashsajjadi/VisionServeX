...
def ev(task):
    ...
    metric = (
        ...
        or r.get("embed_dim")
        or r.get("n_boxes")
        or r.get("n_instances")
        or r.get("frames_tracked")
        or r.get("onnx_bytes")
        or ""
    )
    return {
        ...
        rows.append(
            {
                ...
                "status": r["status"],
                "latency_ms": r.get("latency_ms", ""),
                "metric": e["metric"] if e else "",
                "artifact": r.get("artifact", ""),
                "artifact_exists": artifact_exists(r.get("artifact", "")),
...
SAM_VARIANTS = [
    ...
    (
        ...
        "Apache-2.0",
        True,
        "decoder ONNX export+CPU runtime",
    ),
    (
        "sam-vit-l-onnx",
        None,
        ...
        "Apache-2.0",
        True,
        "ONNX-eligible; needs sam_vit_l .pth checkpoint",
    ),
    (
        "sam-vit-h-onnx",
        None,
        ...
        "Apache-2.0",
        True,
        "ONNX-eligible; needs sam_vit_h .pth checkpoint",
    ),
    ...
    (
        ...
        "Apache-2.0",
        True,
        "efficientvit-sam-l0; ONNX decoder runtime executed",
    ),
    (
        "efficientsam-l1",
        None,
        ...
        "Apache-2.0",
        True,
        "Apache weights; SA-1B dataset provenance documented",
    ),
    (
        "efficientsam-l2",
        None,
    ...
    (
        ...
        "Apache-2.0",
        True,
        "image; HF cached (covered by sam2.1-base-plus run)",
    ),
    ...
    (
        "sam2.1-video-tiny",
        "sam2video:sam2.1-hiera-tiny",
        None,
        "Apache-2.0",
        True,
        "video tracking (propagate_in_video)",
    ),
    (
        "sam2.1-video-small",
        "sam2video:sam2.1-hiera-small",
        None,
        "Apache-2.0",
        True,
        "video tracking",
    ...
    (
        ...
        "Apache-2.0",
        True,
        "video; same backend as video-small (HF cached)",
    ),
    (
        "sam2.1-video-large",
        None,
    ...
    (
        ...
        "Apache-2.0",
        True,
        "SAM2 ONNX exporter not in transformers; isolated-env sidecar path",
    ),
    (
        "sam2.1-onnx-small",
        None,
        ...
        "Apache-2.0",
        True,
        "see sam2.1-onnx-tiny attempt blocker",
    ),
    (
    ...
    (
        ...
        "Apache-2.0",
        True,
        "wanglab/medsam via sam_hf (v3.5 executed)",
    ),
    (
        "medsam2",
        None,
        ...
        "Apache-2.0",
        True,
        "MedSAM2 sidecar; missing preprocessor_config for HF load",
    ),
    (
        "hq-sam",
        None,
        "legal_review_required",
        "Apache-2.0 code / HQSeg-44K NC data",
        False,
        "ThinObject-5K CC-BY-NC, DIS5K NC",
    ),
    (
        "hq-sam2",
        None,
    ...
    (
        ...
        "Apache-2.0 code / HQSeg-44K NC data",
        False,
        "TinyViT; HQSeg-44K inherited NC",
    ),
    (
        "tinysam",
        None,
        ...
        "Apache-2.0",
        True,
        "product_grade_candidate; SA-1B dataset provenance",
    ),
        ...
        rows.append(
            {
                ...
                "artifact_exists": artifact_exists(art),
                "exact_command": cmd,
                "notes": note,
            }
        )
...
DINO_VARIANTS = [
    (
        ...
        "Apache-2.0",
        True,
        "embed dim 384 (alias dinov2-small)",
    ),
    (
        "dinov2-vitb14",
        "dinov2:dinov2-base",
        ...
        "Apache-2.0",
        True,
        "embed dim 768 (alias dinov2-base)",
    ),
    (
        "dinov2-vitl14",
        "dinov2:dinov2-large",
        ...
        "Apache-2.0",
        True,
        "embed dim 1024 (alias dinov2-large)",
    ),
    (
        "dinov2-vitg14",
        "dinov2:dinov2-giant",
        ...
        "Apache-2.0",
        True,
        "embed dim 1536 (alias dinov2-giant)",
    ),
    ...
    (
        ...
        "Apache-2.0",
        True,
        "original DINO SSL ViT-S/8, dim 384",
    ),
    (
        "grounding-dino-tiny",
        "gdino:grounding-dino-tiny",
        None,
        "Apache-2.0",
        True,
        "open-vocab detect",
    ...
    (
        ...
        "Apache-2.0",
        True,
        "swin-t == grounding-dino-tiny",
    ),
    (
        "grounding-dino-swin-b",
        "gdino:grounding-dino-base",
        None,
        "Apache-2.0",
        True,
        "swin-b == grounding-dino-base",
    ),
    (
        "grounding-dino-base",
        "gdino:grounding-dino-base",
...
for _iid in [
    ...
    DINO_VARIANTS.append(
        (
            _iid,
            None,
            "auth_required",
            "DINOv3 License (Meta custom, gated)",
            False,
...
for _iid in [
    ...
    DINO_VARIANTS.append(
        (
            ...
            None,
            "external_api_only",
            "proprietary API (Apache SDK only)",
            False,
...
def _dino_cmd(iid, state):
    ...
    if state == "auth_required":
        return (
            f"export HF_TOKEN=... && visionservex dino status {iid}  # gated DINOv3 custom license"
        )
...
def build_inventory(sam_rows, dino_rows, exec_rows):
    ...
    for r in sam_rows:
        ...
        add(
            ...
            True,
            prod,
            "" if r["executed"] == "YES" else r["notes"],
            r["exact_command"],
        )
    for r in dino_rows:
        prod = "product_grade_pass" if r["final_state"] == "benchmark_passed" else r["final_state"]
        fam = (
            "grounding-dino"
            if r["variant_id"].startswith("grounding")
            else ("dino-x" if r["variant_id"].startswith("dino-x") else "dino")
        )
        add(
            r["variant_id"],
            fam,
    ...
    for iid, task, restask, fam, src, lic in extra:
        ...
        add(
            ...
            lic,
            src,
            e["artifact"] if e else "",
            True,
            False,
            "product_grade_pass",
            "",
            f"visionservex feature embed {iid} image.jpg"
            if "embed" in restask
            else f"# run via transformers ({src})",
        )
    ...
    for v in ["nano", "small", "medium", "large", "xl", "2xl"]:
        ...
        add(
            ...
            True,
            True,
            "product_grade_pass" if st == "benchmark_passed" else "checkpoint_required",
    ...
    for iid in ["ritm", "clickseg", "simpleclick", "focalclick"]:
        ...
        add(
            ...
            csafe,
            d.get("license", "MIT"),
            d.get("official_source", ""),
            "",
            True,
            True,
            "product_grade_candidate" if iid == "ritm" else "legal_review_required",
            d.get("training_data_risk", ""),
            d.get(
                "exact_next_command",
                f"visionservex interactive run {iid} image.jpg --positive-points pos.json",
            ),
        )
    ...
    for iid, fam, lic, src in [
        ...
        st = (
            "excluded_restricted"
            if iid in ("edgesam", "yolov8-seg", "yolo11-seg", "locateanything-3b")
            else "legal_review_required"
        )
        add(
            iid,
            fam,
            "segmentation_or_grounding",
            "v3.6-3.7",
            st,
            False,
            False,
            ...
            "",
            True,
            iid == "locateanything-3b",
            st,
            "excluded from commercial-safe core",
        ...
        json.dumps(
            {
                ...
                "sam_benchmark_passed": sum(
                    1 for r in sam_rows if r["final_state"] == "benchmark_passed"
                ),
                "dino_variants": len(dino_rows),
                "dino_benchmark_passed": sum(
                    1 for r in dino_rows if r["final_state"] == "benchmark_passed"
                ),
