...
def t_sam1_hf(model_id: str):
    ...
    hf = {"sam-vit-base": "facebook/sam-vit-base",
          "sam-vit-large": "facebook/sam-vit-large",
          "sam-vit-huge": "facebook/sam-vit-huge"}[model_id]
    img = _img()
    ...
    return {"engine": "sam_hf/transformers", "hf_id": hf, "task": "promptable_segmentation",
            "mask_shape": list(m.shape), "mask_area": int((m > 0).sum()),
            "latency_ms": round(latency, 1), "artifact": str(npy.relative_to(ROOT))}
...
def t_sam1_onnx(model_id: str):
    ...
    return {"engine": "onnx_export+onnxruntime", "task": "onnx_decoder_export_and_runtime",
            "onnx_bytes": size, "export_ms": round(export_ms, 1), "runtime_ms": round(run_ms, 1),
            "export_result": exp, "run_result": run,
...
def t_mobilesam_seg(_):
    ...
    np.save(npy, m.astype("uint8"))
    return {"engine": "mobile_sam", "task": "promptable_segmentation",
            "mask_shape": list(m.shape), "mask_area": int((m > 0).sum()),
            "score": float(scores[0]), "latency_ms": round(latency, 1),
...
def t_mobilesam_onnx(_):
    ...
    run_ms = (time.perf_counter() - t1) * 1000
    return {"engine": "onnx_export+onnxruntime", "task": "onnx_decoder_export_and_runtime",
            "onnx_bytes": out.stat().st_size, "export_ms": round(export_ms, 1),
            "runtime_ms": round(run_ms, 1), "export_result": exp, "run_result": run,
            "artifact": str(out.relative_to(ROOT))}
...
def t_efficientsam_onnx(_):
    ...
    shapes = {i.name: [d if isinstance(d, int) else str(d) for d in i.shape] for i in sess.get_inputs()}
    ...
    latency = (time.perf_counter() - t0) * 1000
    return {"engine": "onnxruntime", "task": "onnx_decoder_runtime_smoke",
            "onnx_bytes": p.stat().st_size, "input_names": names, "input_shapes": shapes,
            "output_shapes": out_shapes, "ran": ran, "run_error": err,
            "latency_ms": round(latency, 1), "artifact": str(p.relative_to(ROOT))}
...
def t_dinov2(model_id: str):
    ...
    hf = {"dinov2-small": "facebook/dinov2-small", "dinov2-base": "facebook/dinov2-base",
          "dinov2-large": "facebook/dinov2-large", "dinov2-giant": "facebook/dinov2-giant"}[model_id]
    img = _img()
    t0 = time.perf_counter()
    ...
    np.save(npy, emb)
    return {"engine": "dinov2/transformers", "hf_id": hf, "task": "embedding",
            "embed_dim": int(emb.shape[0]), "embed_norm": float(np.linalg.norm(emb)),
            "latency_ms": round(latency, 1), "artifact": str(npy.relative_to(ROOT))}
...
def t_dino_vits8(_):
    ...
    np.save(npy, emb)
    return {"engine": "dino/transformers", "hf_id": hf, "task": "embedding_ssl",
            "embed_dim": int(emb.shape[0]), "embed_norm": float(np.linalg.norm(emb)),
            "latency_ms": round(latency, 1), "artifact": str(npy.relative_to(ROOT))}
...
def t_gdino(model_id: str):
    ...
    hf = {"grounding-dino-tiny": "IDEA-Research/grounding-dino-tiny",
          "grounding-dino-base": "IDEA-Research/grounding-dino-base"}[model_id]
    img = _img()
    t0 = time.perf_counter()
    ...
    payload = {"engine": "grounding_dino/transformers", "hf_id": hf, "task": "open_vocab_detect",
               "n_boxes": len(boxes), "boxes": boxes, "scores": res["scores"].tolist(),
...
def t_clip(_):
    ...
    np.save(npy, emb)
    return {"engine": "clip/transformers", "hf_id": hf, "task": "image_embedding",
            "embed_dim": int(emb.shape[0]), "embed_norm": float(np.linalg.norm(emb)),
            "latency_ms": round(latency, 1), "artifact": str(npy.relative_to(ROOT))}
...
def t_owlvit(_):
    ...
    pp = getattr(proc, "post_process_grounded_object_detection", None) or proc.post_process_object_detection
    res = pp(out, threshold=0.1, target_sizes=tgt)[0]
    latency = (time.perf_counter() - t0) * 1000
    payload = {"engine": "owlvit/transformers", "hf_id": hf, "task": "open_vocab_detect",
               "n_boxes": int(len(res["boxes"])), "scores": res["scores"].tolist()[:10],
               "latency_ms": round(latency, 1)}
...
def t_owlv2(_):
    ...
    res = proc.post_process_grounded_object_detection(
        out, threshold=0.1, target_sizes=torch.tensor([img.size[::-1]]))[0]
    latency = (time.perf_counter() - t0) * 1000
    payload = {"engine": "owlv2/transformers", "hf_id": hf, "task": "open_vocab_detect",
               "n_boxes": int(len(res["boxes"])), "scores": res["scores"].tolist()[:10],
               "latency_ms": round(latency, 1)}
...
def t_depth(_):
    ...
    return {"engine": "depth_anything/transformers", "hf_id": hf, "task": "monocular_depth",
            "depth_shape": list(depth.shape), "depth_min": float(depth.min()),
            "depth_max": float(depth.max()), "latency_ms": round(latency, 1),
            "artifact": str(npy.relative_to(ROOT))}
...
def t_rfdetr_seg(variant: str):
    import numpy as np
    import rfdetr
    cls = {"nano": rfdetr.RFDETRSegNano, "small": rfdetr.RFDETRSegSmall,
           "medium": rfdetr.RFDETRSegMedium, "large": rfdetr.RFDETRSegLarge,
           "xl": rfdetr.RFDETRSegXLarge, "2xl": rfdetr.RFDETRSeg2XLarge}[variant]
    img = _img()
    t0 = time.perf_counter()
    model = cls(device="cpu")
    det = model.predict(img, threshold=0.1)
    latency = (time.perf_counter() - t0) * 1000
    n = int(len(det.xyxy)) if hasattr(det, "xyxy") and det.xyxy is not None else 0
    has_masks = bool(getattr(det, "mask", None) is not None)
    payload = {"engine": "rfdetr", "variant": f"rfdetr-seg-{variant}", "task": "instance_segmentation",
               "n_instances": n, "has_masks": has_masks,
               "class_ids": det.class_id.tolist()[:20] if getattr(det, "class_id", None) is not None else [],
               "latency_ms": round(latency, 1)}
...
def t_pipeline(spec: str):
    ...
    from transformers import AutoProcessor, GroundingDinoForObjectDetection
    gd_hf = {"grounding-dino-tiny": "IDEA-Research/grounding-dino-tiny",
             "grounding-dino-swin-b": "IDEA-Research/grounding-dino-base",
             "grounding-dino-base": "IDEA-Research/grounding-dino-base"}[detector]
    ...
    if segmenter.startswith("sam2"):
        ...
        from transformers import SamModel, SamProcessor
        sam_hf = {"sam-vit-base": "facebook/sam-vit-base", "sam-vit-large": "facebook/sam-vit-large",
                  "sam-vit-huge": "facebook/sam-vit-huge"}[segmenter]
    ...
    latency = (time.perf_counter() - t0) * 1000
    payload = {"engine": "pipeline", "pipeline_id": spec, "task": "text_to_mask",
               "detector": detector, "segmenter": segmenter, "n_boxes": int(len(res["boxes"])),
               "top_box": box, "top_score": float(res["scores"][0]), "mask_area": int(seg_area),
               "prompt_text": "a person. a car.", "latency_ms": round(latency, 1)}
...
def t_sam21_onnx_attempt(_):
    ...
    try:
        ...
        m = Sam2Model.from_pretrained("facebook/sam2.1-hiera-tiny").to("cpu").eval()
        attempts.append({"approach": "torch.onnx.export(Sam2Model)",
    ...
    blocker = {
        "engine": "n/a", "task": "sam21_onnx_export_attempt", "state": "blocked",
        "blocker_code": "SAM2_ONNX_EXPORTER_NOT_AVAILABLE",
        "reason": ("transformers 5.3 Sam2Model has no ONNX export path; official sam2 repo "
        ...
        "attempts": attempts,
        "next_action": ("pip install sam2 (official, Apache-2.0) in an isolated env, run "
...
def t_locateanything(_):
    ...
    payload = {
        "engine": "sidecar(NVlabs/Eagle)", "task": "grounded_detect", "state": "legal_review_required",
        "default_safe": False, "commercial_safe": False, "sidecar_installed": has_eagle,
...
TASKS = {
    ...
    "pipe:grounding-dino-swin-b+sam-vit-huge": lambda: t_pipeline("grounding-dino-swin-b+sam-vit-huge"),
    "pipe:grounding-dino-swin-b+sam-vit-large": lambda: t_pipeline("grounding-dino-swin-b+sam-vit-large"),
    "pipe:grounding-dino-tiny+sam2-hiera-tiny": lambda: t_pipeline("grounding-dino-tiny+sam2-hiera-tiny"),
    "pipe:grounding-dino-swin-b+sam2.1-hiera-small": lambda: t_pipeline("grounding-dino-swin-b+sam2.1-hiera-small"),
    "pipe:grounding-dino-swin-b+sam2.1-hiera-large": lambda: t_pipeline("grounding-dino-swin-b+sam2.1-hiera-large"),
