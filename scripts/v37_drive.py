...
def list_tasks() -> list[str]:
    out = subprocess.run(
        [sys.executable, str(RUNNER), "--task", "LIST"], capture_output=True, text=True
    )
...
def main():
    ...
    env = {
        ...
        "HOME": str(Path.home()),
        "TMPDIR": tmpdir,
    }  # sda2 tmp — protect /tmp tmpfs quota
    import os
    ...
    for i, task in enumerate(tasks, 1):
        ...
        try:
            p = subprocess.run(
                [sys.executable, str(RUNNER), "--task", task],
                capture_output=True,
                text=True,
                timeout=TIMEOUT,
                env=full_env,
            ...
            rec = {"task": task, "status": "timeout", "error": f"exceeded {TIMEOUT}s"}
        ...
        print(
            f"    -> {rec.get('status')} ({rec.get('wall_s')}s) {rec.get('error', '')[:80]}",
            flush=True,
        )
            ...
            w.writerow(
                {
                    ...
                    "model_id": mid,
                    "engine": r.get("engine", ""),
                    "result_task": r.get("task_name") or r.get("result_task") or r.get("task", ""),
                    "status": r.get("status"),
