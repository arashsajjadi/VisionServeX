...
ALIAS = {
    ...
    "medsam-vit-b": "medsam",
    "edge-sam": "edgesam",
    "sam2-image-tiny": "sam2-hiera-tiny",
    "sam2-image-small": "sam2-hiera-small",
    "sam2-image-base-plus": "sam2-hiera-base-plus",
    "sam2-image-large": "sam2-hiera-large",
    "sam2.1-image-tiny": "sam2.1-hiera-tiny",
    "sam2.1-image-small": "sam2.1-hiera-small",
    "sam2.1-image-base-plus": "sam2.1-hiera-base-plus",
    "sam2.1-image-large": "sam2.1-hiera-large",
    "efficientsam-tiny": "efficientsam",
    "efficientsam-small": "efficientsam",
}
...
FACT = {
    ...
    "hq-sam": (
        "Apache-2.0",
        "Apache-2.0 (declared); HQSeg-44K NC training data",
        "public_download",
        "conditional",
    ),
    ...
    "tinysam": (
        "Apache-2.0",
        "Apache-2.0 tag; SA-1B research-only provenance",
        "public_download",
        "conditional",
    ),
    "q-tinysam": (
...
def classify(mid: str) -> dict:
    ...
    return {
        "current_state_before": cur or "absent_from_ledger",
        "target_state_after": target,
        "code_license": code_l,
        "weights_license": weights_l,
...
SAM_TARGETS = {
    ...
    "SAM2": [
        ...
        "sam2-image-tiny",
        "sam2-image-small",
        "sam2-image-base-plus",
        "sam2-image-large",
        "sam2-video-tiny",
        "sam2-video-small",
        "sam2-video-base-plus",
        "sam2-video-large",
    ],
    "SAM2.1": [
        "sam2.1-hiera-tiny",
...
def next_command(mid: str, target: str) -> str:
    if target == "auth_required":
        return (
            f"export HF_TOKEN=... && visionservex sam status {mid}  # request HF gated access first"
        )
    ...
    for gen, mids in targets.items():
        for mid in mids:
            if mid in NOT_RELEASED:
                c = {
                    "current_state_before": "absent_from_ledger",
                    "target_state_after": "not_released",
                    "code_license": "n/a (unreleased)",
                    "weights_license": "n/a (unreleased)",
                    "weights_available": "unavailable",
            ...
            rows.append(
                {
                    ...
                    **c,
                    "evidence_artifact": (
                        LED.get(ALIAS.get(mid, mid), {}).get("evidence_artifact", "")
                        if isinstance(LED.get(ALIAS.get(mid, mid)), dict)
                        else ""
                    ),
                    ...
                    "blocker_if_any": (
                        ...
                        if c["target_state_after"] == "excluded_restricted"
                        else "HF gated access"
                        if c["target_state_after"] == "auth_required"
                        else "API-only"
                        if c["target_state_after"] == "external_api_only"
                        else "not yet released"
                        if c["target_state_after"] == "not_released"
                        else "training-data provenance"
                        if c["target_state_after"] == "legal_review_required"
                        else "ONNX local-export step pending"
                        if c["target_state_after"] == "checkpoint_required"
                        else ""
                    ),
                }
            )
...
for rows, name in [(sam_rows, "v31_sam_family_matrix"), (dino_rows, "v31_dino_family_matrix")]:
    ...
    for r in rows:
        L.append(
            f"| {r['model_id']} | {r['generation']} | {r['current_state_before']} | {r['target_state_after']} | {r['weights_license'][:30]} | {r['exact_user_command'][:50]} |"
        )
...
baseline = {
    "version_before": "v3.0.0",
    "sprint": "v3.1 SAM/DINO model expansion",
    "core_rows": len(led),
    "benchmark_passed": int((led["final_state"] == "benchmark_passed").sum()),
    "final_state_counts": {k: int(v) for k, v in fs.items()},
    "sam_targets_total": len(sam_rows),
    "dino_targets_total": len(dino_rows),
    "pipelines_total": len(pipe_rows),
    "sam_state_dist": dict(
        __import__("collections").Counter(r["target_state_after"] for r in sam_rows)
    ),
    "dino_state_dist": dict(
        __import__("collections").Counter(r["target_state_after"] for r in dino_rows)
    ),
