...
def classify_model(
    final_state: str, default_safe: str, license_status: str, evidence_artifact: str
...
def cli_help_ok(args: list[str]) -> bool:
    try:
        r = subprocess.run(
            [sys.executable, "-m", "visionservex", *args, "--help"],
            capture_output=True,
            text=True,
            timeout=90,
            cwd=ROOT,
        ...
        return r.returncode == 0 and (
            "Usage" in r.stdout or "Commands" in r.stdout or "Options" in r.stdout
        )
    except Exception:
        return False
...
CLI_GROUPS = {
    "sam": cli_help_ok(["sam"]),
    "dino": cli_help_ok(["dino"]),
    "pipeline": cli_help_ok(["pipeline"]),
    "cv2-pro": cli_help_ok(["cv2-pro"]),
    "run": cli_help_ok(["run"]),
    "list-models": cli_help_ok(["list-models"]),
}
    ...
    model_rows.append(
        {
            ...
            "exact_next_command": m.get("next_iteration_command", "")
            or m.get("manual_fix_command", ""),
        }
    )
...
for t in load("smart_tool_coverage_ledger.csv"):
    ...
    tool_rows.append(
        {
            "tool_id": t["tool_id"],
            "tool_kind": "classic-smart",
            "final_state": fs,
            "dependency_license": t.get("dependency_license", ""),
...
def family_matrix_stats(name):
    ...
    return {
        "total": n,
        "pass": cats.count("PASS"),
        "blocked": cats.count("BLOCKED"),
        "excluded": cats.count("EXCLUDED"),
...
summary = {
    ...
    "models_blocked": n_block,
    "models_excluded": n_excl,
    "models_other_unverified": n_other,
    ...
    "pipeline_total": n_pipe,
    "pipeline_pass": n_pipe_pass,
    "pipeline_pass_pct": pct(n_pipe_pass, n_pipe),
    "tool_total": n_tool,
    "tool_pass": n_tool_pass,
    "tool_pass_pct": pct(n_tool_pass, n_tool),
    "sam_comprehensive": sam_stats,
    "dino_comprehensive": dino_stats,
    "sidecar_total": sidecar_total,
    "sidecar_pass": sidecar_pass,
    "sidecar_blocked": n_sidecar,
    "sidecar_pass_pct": pct(sidecar_pass, sidecar_total),
    "sidecar_blocked_pct": pct(n_sidecar, sidecar_total),
    "cli_groups_help_ok": CLI_GROUPS,
    "root_cli_ok": ROOT_CLI_OK,
    "python_api_ok": API_OK,
    "bad_license_default_safe_rows": sum(
        1
        for r in model_rows
        if r["category"] != "EXCLUDED" and _has_restricted_license(r["license_status"])
    ),
...
release_rows = [
    ...
    {
        "item": "default_safe_pass_pct",
        "value": summary["default_safe_pass_pct"],
        "of_total_rows": len(ds_rows),
    },
    {
        "item": "evidence_completeness_pct",
        "value": summary["evidence_completeness_pct"],
        "of_total_rows": pass_rows_total,
    },
    ...
    {
        "item": "bad_license_default_safe_rows",
        "value": summary["bad_license_default_safe_rows"],
        "of_total_rows": total,
    },
]
...
# markdown baseline
md = [
    f"""# VisionServeX v3.3 Truth Audit — Baseline (deterministic from canonical ledgers)
...
print(
    f"SAM (v31 {sam_stats['total']}): pass {sam_stats['pass_pct']}% blk {sam_stats['blocked_pct']}% exc {sam_stats['excluded_pct']}%"
)
print(
    f"DINO (v31 {dino_stats['total']}): pass {dino_stats['pass_pct']}% blk {dino_stats['blocked_pct']}% exc {dino_stats['excluded_pct']}%"
)
print(f"SIDECAR: pass {sidecar_pass}/{sidecar_total} = {summary['sidecar_pass_pct']}%")
print(
    f"CLI root={ROOT_CLI_OK} groups_ok={sum(CLI_GROUPS.values())}/{len(CLI_GROUPS)}  API={API_OK}"
)
print(f"bad-license default-safe rows: {summary['bad_license_default_safe_rows']}")
print(
    "evidence artifacts NOT found on disk for PASS rows:",
    [r["model_id"] for r in model_rows if r["counts_as_pass"] and not r["artifact_exists_on_disk"]][
        :20
    ],
)
print("=" * 60)
