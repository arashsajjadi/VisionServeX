...
SAM_RUNNABLE = ["sam-vit-h", "sam2-hiera-large", "sam2.1-hiera-small", "mobilesam", "efficientsam", "medsam"]
SAM_GATED = ["sam3-base"]
...
for m in SAM_GATED:
    nb = make_nb(f"SAM tutorial — {m} (BYOT / gated)",
    ...
    written.append(write("sam_family", m, nb))
for m in SAM_LEGAL:
    nb = make_nb(f"SAM tutorial — {m} (legal review)",
...
for m in SAM_EXCLUDED:
    nb = make_nb(f"SAM tutorial — {m} (NON-COMMERCIAL, excluded)",
...
DINO_GATED = [("dinov3-vitb16", "legal"), ("grounding-dino-1.5", "auth"), ("grounding-dino-1.6", "auth"),
              ("dino-x-api", "api")]
for m, kind in DINO_RUN:
    nb = make_nb(f"DINO tutorial — {m}",
...
for m, kind in DINO_GATED:
    note = {"legal": "DINOv3 custom Meta license (HF-gated) — license-aware, not Apache.",
            "auth": "GroundingDINO 1.5/1.6 are API/token-gated — BYOT.",
            "api": "DINO-X is API-only — external; weights never mirrored."}[kind]
    nb = make_nb(f"DINO tutorial — {m} ({kind})", f"**{m}**: {note}",
        [f'from visionservex import VSX\nimport json; print(json.dumps(VSX.dino("{m}").explain(), indent=2, default=str))'])
    written.append(write("dino_family", m, nb))

# ---- pipelines (text-to-mask) ----
PIPES = ["grounding-dino-swin-t+sam2.1-hiera-small", "grounding-dino-swin-b+sam2.1-hiera-large",
         "grounding-dino-swin-t+sam-vit-h", "grounding-dino-original-swin-t+sam2-hiera-small",
         "grounding-dino-1.6+sam3-base", "dinov3-vitb16+sam3-base"]
for p in PIPES:
    nb = make_nb(f"Pipeline tutorial — {p}",
...
for tool in _t.list_tools():
    ...
    nb = make_nb(f"CV2-Pro tutorial — {tool}",
...
# ---- interactive-seg ----
for m, state in [("ritm", "checkpoint_required"), ("clickseg", "checkpoint_required"),
                 ("focalclick", "legal_review_required"), ("simpleclick", "legal_review_required")]:
    nb = make_nb(f"Interactive-seg tutorial — {m} ({state})",
