...
# --------------------------------------------------------------------------- #
def _core(model_id, family, *, code="Apache-2.0", weights="Apache-2.0",
          dataset_risk="low", hf_repo="", upstream="", nxt="", notes="",
          aliases=()):
    ...
    """
    return ModelPolicy(
        model_id=model_id, family=family, final_policy="commercial_safe_core",
        code_license=code, weights_license=weights, dataset_risk=dataset_risk,
        gated=False, local_token_required=False, user_license_required=False,
        can_auto_download=True, can_ship_weights=False,
        default_safe=True, commercial_safe=True, production_allowed=True,
        is_local=True, hf_repo=hf_repo, upstream_url=upstream,
        exact_next_command=nxt or f"visionservex model pull {model_id}",
        notes=notes, aliases=tuple(aliases),
    )


def _byot(model_id, family, *, code, weights, dataset_risk="medium", hf_repo,
          upstream="", nxt="", notes="", aliases=()):
    """byot_license_required: gated, user supplies token + accepts upstream license."""
    return ModelPolicy(
        model_id=model_id, family=family, final_policy="byot_license_required",
        code_license=code, weights_license=weights, dataset_risk=dataset_risk,
        gated=True, local_token_required=True, user_license_required=True,
        can_auto_download=False, can_ship_weights=False,
        default_safe=False, commercial_safe=False, production_allowed=False,
        is_local=True, hf_repo=hf_repo, upstream_url=upstream,
        exact_next_command=(
            nxt
...
def _api(model_id, family, *, code="proprietary (hosted API)", weights="not released",
         provider_env="DINOX_API_KEY", upstream="", nxt="", notes="", aliases=()):
    ...
    return ModelPolicy(
        ...
        code_license=code, weights_license=weights, dataset_risk="n/a",
        gated=False, local_token_required=False, user_license_required=True,
        can_auto_download=False, can_ship_weights=False,
        default_safe=False, commercial_safe=False, production_allowed=False,
        is_local=False, hf_repo="", upstream_url=upstream,
        exact_next_command=nxt or f"export {provider_env}=... && visionservex model license {model_id}",
        notes=notes, aliases=tuple(aliases),
    )


def _noncommercial(model_id, family, *, code, weights, dataset_risk="high",
                   gated=False, hf_repo="", upstream="", nxt="", notes="", aliases=()):
    """noncommercial_restricted: never production / default-safe / commercial."""
    return ModelPolicy(
        model_id=model_id, family=family, final_policy="noncommercial_restricted",
        code_license=code, weights_license=weights, dataset_risk=dataset_risk,
        gated=gated, local_token_required=gated, user_license_required=True,
        can_auto_download=False, can_ship_weights=False,
        default_safe=False, commercial_safe=False, production_allowed=False,
        is_local=True, hf_repo=hf_repo, upstream_url=upstream,
        exact_next_command=nxt or f"visionservex model license {model_id}  # non-commercial; research-only",
        notes=notes, aliases=tuple(aliases),
    )


def _enterprise(model_id, family, *, code, weights, upstream="", nxt="", notes="",
                aliases=()):
    """enterprise_license_required: AGPL / enterprise-only; disabled in core."""
    return ModelPolicy(
        model_id=model_id, family=family, final_policy="enterprise_license_required",
        code_license=code, weights_license=weights, dataset_risk="medium",
        gated=False, local_token_required=False, user_license_required=True,
        can_auto_download=False, can_ship_weights=False,
        default_safe=False, commercial_safe=False, production_allowed=False,
        is_local=True, hf_repo="", upstream_url=upstream,
        exact_next_command=nxt or f"visionservex model license {model_id}  # enterprise/AGPL license required",
        notes=notes, aliases=tuple(aliases),
    )


def _legal(model_id, family, *, code, weights, dataset_risk="high", gated=False,
           hf_repo="", upstream="", nxt="", notes="", aliases=()):
    """legal_review_required: provenance unclear; not commercial-safe until resolved."""
    return ModelPolicy(
        model_id=model_id, family=family, final_policy="legal_review_required",
        code_license=code, weights_license=weights, dataset_risk=dataset_risk,
        gated=gated, local_token_required=gated, user_license_required=True,
        can_auto_download=False, can_ship_weights=False,
        default_safe=False, commercial_safe=False, production_allowed=False,
...
def _not_released(model_id, family, *, notes="", upstream="", aliases=()):
    ...
    return ModelPolicy(
        model_id=model_id, family=family, final_policy="not_released_or_unverifiable",
        code_license="unknown", weights_license="unknown", dataset_risk="unknown",
        gated=False, local_token_required=False, user_license_required=False,
        can_auto_download=False, can_ship_weights=False,
        default_safe=False, commercial_safe=False, production_allowed=False,
        is_local=True, hf_repo="", upstream_url=upstream,
...
_ROWS += [
    ...
    _core("sam-vit-base", "sam", hf_repo="facebook/sam-vit-base",
          upstream=f"{_GH}/facebookresearch/segment-anything", dataset_risk="low",
          aliases=("sam-vit-b",)),
    _core("sam-vit-large", "sam", hf_repo="facebook/sam-vit-large",
          aliases=("sam-vit-l",)),
    _core("sam-vit-huge", "sam", hf_repo="facebook/sam-vit-huge",
          aliases=("sam-vit-h",)),
    _core("mobilesam", "sam", upstream=f"{_GH}/ChaoningZhang/MobileSAM",
          notes="MobileSAM Apache-2.0; ONNX-exportable."),
    _core("efficientsam", "sam", upstream=f"{_GH}/yformer/EfficientSAM",
    ...
    _core("grounding-dino-tiny", "grounding-dino",
          hf_repo="IDEA-Research/grounding-dino-tiny"),
    _core("grounding-dino-base", "grounding-dino",
          hf_repo="IDEA-Research/grounding-dino-base"),
    _core("grounding-dino-swin-t", "grounding-dino",
          upstream=f"{_GH}/IDEA-Research/GroundingDINO"),
    _core("grounding-dino-swin-b", "grounding-dino",
          upstream=f"{_GH}/IDEA-Research/GroundingDINO"),
    # Florence-2 (MIT)
    _core("florence-2-base", "florence2", code="MIT", weights="MIT",
          hf_repo="microsoft/Florence-2-base"),
    _core("florence-2-large", "florence2", code="MIT", weights="MIT",
          hf_repo="microsoft/Florence-2-large"),
    # CLIP (MIT/OpenAI), OWL-ViT / OWLv2 (Apache-2.0)
    _core("clip-vit-base-patch32", "clip", code="MIT", weights="MIT",
          hf_repo="openai/clip-vit-base-patch32", aliases=("clip",)),
    _core("owlvit-base-patch32", "owlvit", hf_repo="google/owlvit-base-patch32",
          aliases=("owl-vit",)),
    _core("owlv2-base-patch16-ensemble", "owlv2",
          hf_repo="google/owlv2-base-patch16-ensemble", aliases=("owlv2",)),
    # DepthAnything small only (Apache-2.0). Larger V2 -> non-commercial below.
    _core("depth-anything-small", "depth-anything",
    ...
    _core("rfdetr-seg-nano", "rf-detr", upstream=f"{_GH}/roboflow/rf-detr",
          dataset_risk="low"),
    ...
    _core("efficientvit-sam-l0", "efficient-sam", upstream=f"{_GH}/mit-han-lab/efficientvit",
          dataset_risk="medium",
          notes="Apache-2.0 weights; SA-1B research-only dataset provenance documented."),
    _core("efficientvit-sam-l1", "efficient-sam", upstream=f"{_GH}/mit-han-lab/efficientvit",
          dataset_risk="medium"),
    _core("efficientvit-sam-l2", "efficient-sam", upstream=f"{_GH}/mit-han-lab/efficientvit",
          dataset_risk="medium"),
    # RITM interactive segmentation (MIT code + permissive datasets)
    _core("ritm", "interactive-seg", code="MIT", weights="MIT", dataset_risk="low",
...
for _mid in ("sam3-base", "sam3-image", "sam3-video", "sam3-text-prompt",
             "sam3-visual-prompt", "sam3-exemplar-prompt", "sam3-open-vocabulary",
             "sam3-tracking"):
    _ROWS.append(_byot(_mid, "sam3", code=_SAM3_LIC, weights=_SAM3_LIC,
                       hf_repo="facebook/sam3", upstream=_SAM3_URL,
...
for _mid in ("sam3.1-base", "sam3.1-image", "sam3.1-video", "sam3.1-open-vocabulary",
             "sam3.1-text-prompt", "sam3.1-visual-prompt", "sam3.1-real-time-tracking"):
    _ROWS.append(_byot(_mid, "sam3.1", code=_SAM3_LIC, weights=_SAM3_LIC,
                       hf_repo="facebook/sam3.1", upstream=_SAM31_URL,
...
_dinov3 = {
    ...
    "dinov3-vit7b16": ("facebook/dinov3-vit7b16-pretrain-lvd1689m", "ViT-7B/16 ~6.8B (large footprint)"),
    "dinov3-convnext-tiny": ("facebook/dinov3-convnext-tiny-pretrain-lvd1689m", "ConvNeXt-Tiny ~29M"),
    "dinov3-convnext-small": ("facebook/dinov3-convnext-small-pretrain-lvd1689m", "ConvNeXt-Small ~50M"),
    "dinov3-convnext-base": ("facebook/dinov3-convnext-base-pretrain-lvd1689m", "ConvNeXt-Base ~119M"),
    "dinov3-convnext-large": ("facebook/dinov3-convnext-large-pretrain-lvd1689m", "ConvNeXt-Large ~198M"),
}
for _mid, (_repo, _desc) in _dinov3.items():
    _ROWS.append(_byot(_mid, "dinov3", code=_DINOV3_LIC, weights=_DINOV3_LIC,
                       hf_repo=_repo, upstream=f"https://huggingface.co/{_repo}",
...
_ROWS += [
    _api("grounding-dino-1.5", "grounding-dino",
         code="Apache-2.0 (client SDK only)", provider_env="DDS_API_KEY",
         upstream=f"{_GH}/IDEA-Research/Grounding-DINO-1.5-API",
         notes="Cloud-only (DeepDataSpace); weights proprietary."),
    _api("grounding-dino-1.5-pro", "grounding-dino",
         code="Apache-2.0 (client SDK only)", provider_env="DDS_API_KEY",
         upstream=f"{_GH}/IDEA-Research/Grounding-DINO-1.5-API",
         notes="API-only, no weights. Commercial via paid quota + API ToS."),
    _api("grounding-dino-1.6-pro", "grounding-dino",
         code="Apache-2.0 (client SDK only)", provider_env="DDS_API_KEY",
         upstream=f"{_GH}/IDEA-Research/Grounding-DINO-1.5-API",
         notes="SOTA variant. API-only, no weights."),
    _api("dino-x-api", "dino-x", upstream=f"{_GH}/IDEA-Research/DINO-X-API",
...
# ----- noncommercial_restricted ----------------------------------------------
_ROWS += [
    _noncommercial("edge-sam", "efficient-sam", code="NTU S-Lab License 1.0",
                   weights="NTU S-Lab License 1.0 (non-commercial)", dataset_risk="high",
                   upstream=f"{_GH}/chongzhou96/EdgeSAM",
                   notes="Authors' own license is non-commercial. Commercial only via NTUitive.",
                   aliases=("edgesam",)),
    _noncommercial("locate-anything-3b", "vlm-grounding",
                   code="NVIDIA License", weights="NVIDIA License (non-commercial)",
                   gated=True, hf_repo="nvidia/LocateAnything-3B",
    ...
    _noncommercial("describe-anything-3b", "vlm-grounding",
                   code="NVIDIA License", weights="NVIDIA License (non-commercial)",
                   gated=True, hf_repo="nvidia/DAM-3B",
                   upstream="https://huggingface.co/nvidia/DAM-3B",
                   notes="NVIDIA Describe Anything Model (DAM): non-commercial research license.",
                   aliases=("dam", "dam-3b")),
    _noncommercial("medsam2", "sam", code="Apache-2.0 (code)",
                   weights="non-commercial (medical dataset provenance)", dataset_risk="high",
    ...
    _noncommercial("depth-anything-v2-large", "depth-anything",
                   code="Apache-2.0 (code)", weights="CC-BY-NC-4.0",
    ...
    _noncommercial("simpleclick", "interactive-seg", code="MIT (code)",
                   weights="encumbered (MAE CC-BY-NC backbone)", dataset_risk="high",
    ...
    _noncommercial("focalclick", "interactive-seg", code="MIT (code)",
...
_ROWS += [
    _enterprise("fastsam-s", "fastsam", code="Apache-2.0 (CASIA repo)",
    ...
    _enterprise("fastsam-x", "fastsam", code="Apache-2.0 (CASIA repo)",
                weights="AGPL-3.0 coupling (ultralytics YOLOv8x-seg dependency)",
                upstream=f"{_GH}/CASIA-IVA-Lab/FastSAM"),
    _enterprise("yolov8-seg", "ultralytics", code="AGPL-3.0", weights="AGPL-3.0",
                upstream=f"{_GH}/ultralytics/ultralytics",
                notes="AGPL-3.0 copyleft; commercial needs Ultralytics Enterprise License."),
    _enterprise("yolo11-seg", "ultralytics", code="AGPL-3.0", weights="AGPL-3.0",
                upstream=f"{_GH}/ultralytics/ultralytics",
                notes="AGPL-3.0 copyleft; commercial needs Ultralytics Enterprise License."),
]
...
_ROWS += [
    _legal("hq-sam", "sam-hq", code="Apache-2.0 (code)",
           ...
           notes="HQSeg-44K includes ThinObject-5K (CC-BY-NC) + DIS5K (NC ToU). "
                 "Dataset-to-weights inheritance unsettled.", aliases=("sam-hq",)),
    _legal("light-hq-sam", "sam-hq", code="Apache-2.0 (code)",
           weights="Apache-2.0 weights / HQSeg-44K dataset partly NC",
           upstream=f"{_GH}/SysCV/sam-hq",
           notes="TinyViT backbone; same HQSeg-44K dataset concern as hq-sam."),
    _legal("sam-hq2", "sam-hq", code="Apache-2.0 (code)",
           weights="Apache-2.0 weights / HQSeg-44K dataset partly NC",
           upstream=f"{_GH}/SysCV/sam-hq",
           notes="HQ-SAM-2 (image+video). 'hq-sam2' is an alias of this canonical id.",
           aliases=("hq-sam2",)),
    _legal("tinysam", "efficient-sam", code="Apache-2.0", weights="Apache-2.0",
           dataset_risk="medium", upstream=f"{_GH}/xinghaochen/TinySAM",
    ...
    _legal("q-tinysam", "efficient-sam", code="Apache-2.0", weights="Apache-2.0",
           dataset_risk="medium", upstream=f"{_GH}/xinghaochen/TinySAM",
           notes="W8A8-quantized TinySAM; same SA-1B provenance question."),
    _legal("clickseg", "interactive-seg", code="MIT (code)",
    ...
    _legal("oneformer", "oneformer", code="MIT (code)",
    ...
    _legal("internimage", "internimage", code="MIT (code)",
    ...
    _legal("medsam", "sam", code="Apache-2.0 (code)",
    ...
    _legal("rfdetr-seg-xl", "rf-detr", code="Apache-2.0 (per v3.7 research)",
           weights="Apache-2.0 (seg) — verify not PML-1.0 detection-XL", dataset_risk="low",
    ...
    _legal("rfdetr-seg-2xl", "rf-detr", code="Apache-2.0 (per v3.7 research)",
           weights="Apache-2.0 (seg) — verify not PML-1.0 detection-2XL", dataset_risk="low",
           upstream=f"{_GH}/roboflow/rf-detr",
...
# ----- not_released_or_unverifiable ------------------------------------------
_ROWS += [
    _not_released("grounding-dino-2", "grounding-dino",
