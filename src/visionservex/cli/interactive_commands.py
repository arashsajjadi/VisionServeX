...
app = typer.Typer(name="interactive", help="Click-based interactive segmentation.", no_args_is_help=True)
console = Console()
...
def _load_points(spec: Optional[str]):
...
def cmd_list(fmt: str = typer.Option("table", "--format")) -> None:
    ...
    for c in ("model_id", "state", "license", "commercial_safe"):
        t.add_column(c)
    for r in rows:
        t.add_row(r["model_id"], str(r.get("state")), str(r.get("license", ""))[:48],
...
def cmd_status(
    ...
    from visionservex.interactive_runtime import explain as _ex
    info = _ex(model_id)
    payload = info if explain else {"model_id": model_id, "state": info.get("state"),
...
def cmd_run(
    ...
    positive_points: Optional[str] = typer.Option(None, "--positive-points",
                                                   help="JSON file or inline list of [x,y]."),
    negative_points: Optional[str] = typer.Option(None, "--negative-points",
                                                   help="JSON file or inline list of [x,y]."),
    ...
    from visionservex.interactive_runtime import explain as _ex, run_interactive
    if explain:
    ...
    try:
        res = run_interactive(model_id, Image.open(image).convert("RGB"),
                              positive_points=pos, negative_points=neg)
    except Exception as e:
        res = {"model_id": model_id, "status": "expected_blocker",
               "code": "INTERACTIVE_RUNTIME_BLOCKER", "message": str(e)}
    ...
    if out:
        ...
        (Path(out) / f"{model_id}_result.json").write_text(json.dumps(payload, indent=2, default=str))
    typer.echo(json.dumps(payload, indent=2, default=str))
