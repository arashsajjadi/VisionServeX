...
def test_redact_function():
    ...
    assert result == "hf_***IJ", (
        f"Expected 'hf_***IJ', got {result!r}"
    )
...
def test_no_full_token_in_sam3_status_output(monkeypatch):
    ...
    assert fake_token not in serialised, (
        f"Full HF token found in status output: {serialised!r}"
    )
...
def test_deepdataspace_key_redacted(monkeypatch):
    ...
    assert fake_key not in redacted, (
        f"Full API key leaked in redacted output: {redacted!r}"
    )
    assert "***" in redacted, (
        f"Expected masking '***' in redacted key, got {redacted!r}"
    )
