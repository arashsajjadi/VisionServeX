...
def test_sam2_hiera_tiny_actually_ran():
    ...
    assert data["sam2-hiera-tiny"].get("status") == "ok", f"sam2-hiera-tiny not ok: {data['sam2-hiera-tiny']}"
...
def test_new_execution_ledger_has_15_rows():
    ...
    assert len(new_rows) >= 15, f"Need >=15 new executions, got {len(new_rows)}: {[r['model_id'] for r in new_rows]}"
