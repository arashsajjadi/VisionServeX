...
BAD_TOKENS = ["AGPL", "GPL", "non-commercial", "NON-COMMERCIAL", "NC", "proprietary",
              "research-only", "S-Lab", "NVIDIA License", "DINOv3 License", "SAM License"]
    ...
    for r in _matrix("v37_dino_variant_matrix.csv", "variant_id"):
        if str(r["commercial_safe"]).lower() == "true":
            ...
            assert "DINOv3 License" not in lic and "proprietary" not in lic.lower(), \
                f"{r['variant_id']} commercial_safe but license={lic}"
            assert lic == "Apache-2.0", f"{r['variant_id']} commercial-safe must be Apache, got {lic}"
...
def test_inventory_commercial_safe_only_permissive():
    for r in _matrix("v37_post_v259_inventory.csv", "item_id"):
        if str(r["commercial_safe"]).lower() == "true":
            ...
            for tok in ["AGPL", "non-commercial", "NVIDIA License", "DINOv3 License",
                        "S-Lab", "proprietary"]:
                assert tok not in lic, f"{r['item_id']} commercial_safe with bad license token {tok!r}: {lic}"
